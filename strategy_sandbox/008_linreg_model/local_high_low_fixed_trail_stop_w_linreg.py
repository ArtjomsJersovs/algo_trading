...
class IterativeBacktest(IterativeBase):
    ...
    def calculate_pinbar_strategy(self, lookback_bars=100, ma_interval=20, tp_coef = 1, sl_coef = 1):
        #print header
        # print(75 * "-")
        ...
        self.data['RSI'] = tm.rsi(self.data.close, lookback_bars, fillna=True)
        ...
        breaks_local_high = (self.data[f'max_price_{lookback_bars}'].shift(1)<=self.data.close) & (self.data[f'max_price_{lookback_bars}']>self.data.close.shift(1))
        breaks_local_low = (self.data[f'min_price_{lookback_bars}'].shift(1)>=self.data.close) & (self.data[f'min_price_{lookback_bars}']<self.data.close.shift(1))
        RSI_neutral = (self.data.RSI >= 40) & (self.data.RSI <=60)
        #prepare fetures for linreg model
        ...
        for lag in range(1, lags + 1):
            col = "lag{}".format(lag)
            self.data[col] = bc.data.returns.shift(lag)
            cols.append(col)
        self.data.dropna(inplace = True)
        #for bar in tqdm(range(len(self.data)-1)): # all bars except of last one
        for bar in range(len(self.data)-1):     
            self.pos_result = 0
        #ADD LIN REG MODEL RETURNS PREDICTION
            if bar <500:
                self.prediction = 0
            else:
                model_data = self.data[bar-500:bar].copy()
                lm.fit(model_data[cols].values, model_data.returns)
                model_data = model_data.rename({'returns': 'lag1', 'lag1': 'lag2','lag2':'lag3','lag3':'lag4','lag4':'lag5'}, axis=1).iloc[: , :-1]
                self.prediction = np.sign(lm.predict(model_data[cols][-1:].values)[0])      
            self.data['prediction'].iloc[bar] = self.prediction

        #OPEN POSITIONS
            #if ratio_body_flag.iloc[bar] and ratio_high_flag.iloc[bar] and over_atr_flag.iloc[bar] and over_vol_flag.iloc[bar]:
            if breaks_local_low.iloc[bar] and RSI_neutral.iloc[bar] and self.prediction == 1:
                if self.position == 0:
            ...
            elif breaks_local_high.iloc[bar] and RSI_neutral.iloc[bar] and self.prediction == -1:
                if self.position == 0:
...
bc.calculate_pinbar_strategy(lookback_bars=20, ma_interval = 10, tp_coef=1, sl_coef=2)
bc.plot_data()
bc.data.prediction.value_counts()
...
lookback_bars = list(range(40,100,20))
ma_interval = list(range(10,30,10))
tp_coef = list(range(1,4,1))
sl_coef = list(range(1,3,1))

all_combinations = list(itertools.product(lookback_bars,ma_interval, tp_coef, sl_coef))
start_time = time.time()
counter = 1
for i in all_combinations:
    bc.calculate_pinbar_strategy(lookback_bars=i[0], ma_interval = i[1], tp_coef=i[2], sl_coef=i[3])
    df = df.append(
        {
        'combination':i,
