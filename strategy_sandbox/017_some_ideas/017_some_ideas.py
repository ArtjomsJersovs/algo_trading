...
import os
#os.chdir(os.getcwd()+'/017_some_ideas')
...
#INIT API
client = sf.setup_api_conn_binance()
...
class IterativeBase():
    ...
    def close_all(self, bar, commission = 0.001):
        ...
        print("Performance minus commissions in %: {} | absolute: {}$".format(round(perf_w_comm,2),round(perf_abs,2)))
...
class IterativeBacktest(IterativeBase):
    ...
    def calculate_strategy(self, ma_interval = 15, ATR_incr=5, vol_incr=5, sl_coef = 1, tp_perc=0.49):
        ...
        self.data['pos_returns'] = 0

        #strategy specific
        self.data['rng_stop'] = abs(self.data.high-self.data.low)*tp_perc
        self.data['rng'] = abs(self.data.high-self.data.low)
        self.data['direction'] = np.where((self.data.open-self.data.close)>0,-1,1)
        ...
        for bar in tqdm(range(len(self.data)-1)):
            self.pos_result = 0
        #OPEN POSITIONS
            if self.data.direction.iloc[bar]==-1 and (self.data.ATR.iloc[bar-1]*ATR_incr)<self.data.rng.iloc[bar] and (self.data.volume_ma.iloc[bar-1]*vol_incr)<self.data.volume.iloc[bar]:
                if self.position == 0:
                    self.go_long(bar,amount = 'all')
                    self.position = 1
                    self.data['position'].iloc[bar] = self.position
                    self.start_price = self.data.close.iloc[bar]  
                    self.min_price = self.data.low.iloc[bar]
                    self.max_price = self.data.high.iloc[bar]
                               
            elif self.data.direction.iloc[bar]==1 and (self.data.ATR.iloc[bar-1]*ATR_incr)<self.data.rng.iloc[bar] and (self.data.volume_ma.iloc[bar-1]*vol_incr)<self.data.volume.iloc[bar]:
                if self.position == 0:
        ...
        #CLOSE POSITIONS
            if self.position == 1:
                if self.data.close.iloc[bar]>(self.data.rng_stop.iloc[bar]+self.min_price):
                ...
                elif (self.min_price-(self.data.ATR.iloc[bar]*sl_coef))>self.data.close.iloc[bar]:
                    self.close_long(bar, size='all')
                    self.pos_result = np.where(self.data.close.iloc[bar]>self.start_price,1,-1)
                    self.position = 0
                    self.data['position'].iloc[bar] = self.position
                ...
                if self.data.close.iloc[bar]<(self.max_price-self.data.rng_stop.iloc[bar]):
                    self.close_long(bar, size='all')
                    self.pos_result = np.where(self.data.close.iloc[bar]<self.start_price,1,-1)
                    self.position = 0
                    self.data['position'].iloc[bar] = self.position
                ...
                #if no actions with position, then pull up stoploss after price climbed over pinbar high + ATR
                elif (self.max_price+(self.data.ATR.iloc[bar]*sl_coef))<self.data.close.iloc[bar]:
                    self.close_long(bar, size='all')
                    self.pos_result = np.where(self.data.close.iloc[bar]<self.start_price,1,-1)
                    self.position = 0
                    self.data['position'].iloc[bar] = self.position
            ...
            #index_value = self.data.iloc[bar].index[0]
            self.data['pos_result'].iloc[bar] = self.pos_result
            
            if self.pos_result==1:
                self.data['pos_returns'].iloc[bar] = np.log(self.data.close.iloc[bar].astype(float)/self.start_price)*self.data['position'].iloc[bar]
            else:
                self.data['pos_returns'].iloc[bar] = 0
                
        self.close_all(bar+1) # close pos at the last bar       

bc = IterativeBacktest("ETHUSDT","2023-05-01","2023-08-21",tf='5m',amount = 1000)

bc.calculate_strategy(ma_interval = 30, ATR_incr=8, vol_incr=5, sl_coef = 1, tp_perc=0.44)
bc.plot_data()

#sf.excel_export(bc.data, name = 'test')
...
ma_interval = 30#list(range(15,30,5))
atr_interval = list(range(5,9,1))
vol_size = list(range(5,9,1))
sl_coef = 1#list(np.arange(1,2.25,0.5))
tp_perc = list(np.arange(0.36,0.50,0.04))

all_combinations = list(itertools.product(atr_interval, vol_size, tp_perc))
pairs = ['ETHUSDT','ADAUSDT','AGIXUSDT','AMBUSDT','APEUSDT','APTUSDT','AVAXUSDT','CVXUSDT','DOTUSDT','ETCUSDT','FILUSDT','FTMUSDT','GALAUSDT','GALUSDT','GMTUSDT','HIGHUSDT','LDOUSDT','LINKUSDT','LTCUSDT','MATICUSDT','NEARUSDT','PHBUSDT','SANDUSDT','SOLUSDT','TLMUSDT','TRXUSDT','UNIUSDT','WAVESUSDT']
# for tick in pairs:
#     sf.get_stored_data_close(tick,'1h',"2022-11-01","2023-04-15")

data_to_append = []

for ticker in pairs[0]:
bc = IterativeBacktest(ticker,"2023-08-01","2023-08-28",tf='5m',amount = 1000)
df = pd.DataFrame(columns =['combination' ,'accuracy','perf','perf_w_comm', 'trades' ])
start_time = time.time()
counter = 1
for i in all_combinations:
    #bc.calculate_strategy(ma_interval = i[0], ATR_incr=i[1], vol_incr=i[2], sl_coef=i[3], tp_perc=i[4])
    bc.calculate_strategy(ma_interval = 30, ATR_incr=i[0], vol_incr=i[1], sl_coef=1, tp_perc=i[2])
# Create a dictionary for the row you want to append
    row_data = {
        ...
        'perf_w_comm': (bc.current_balance - bc.initial_balance - bc.costs) / bc.initial_balance * 100,
        'trades': bc.trades
    }
    ...
    print(f'{counter} / {len(pairs)} done...')
    counter += 1
    opt = df.iloc[np.argmax(df.perf_w_comm)]
    print(75*"-")
    print('The Best combination is: \n{}'.format(opt))
    print(f'time spent on gridsearch in minutes: {round((time.time() - start_time)/60,2)}')
df.to_csv('C:/Users/HP/Documents/GitHub/algo_trading/strategy_sandbox/017_some_ideas/all_pairs_test_results.csv')
