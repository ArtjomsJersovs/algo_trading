...
class IterativeBacktest(IterativeBase):
    ...
    def calculate_pinbar_strategy(self, lookback_bars=100, ma_interval=20, body_size=0.30, extremum_ratio_size=0.35, bar_seq = 3, atr = 1):
        #print header
        print(75 * "-")
        ...
        #Calculate pinbar 
        self.data['rng'] = self.data.high-self.data.low
        ...
        #new increase strategy
        # self.data['increase'] = np.where((self.data.rng.shift(1)<self.data.rng) & ((np.sign(self.data.rng.shift(1))>0)&(np.sign(self.data.rng)>0)),self.data.increase.shift(1) + 1,
        #                             np.where((self.data.rng.shift(1)>self.data.rng) & ((np.sign(self.data.rng.shift(1))<0)&(np.sign(self.data.rng)<0)),self.data.increase.shift(1) -1,0))
        ...
        for bar in tqdm(range(len(self.data)-1)): # all bars except of last one
            ...
            if (self.data.rng.iloc[bar-1]<self.data.rng.iloc[bar]) and (np.sign(self.data.rng.iloc[bar-1])>0) and (np.sign(self.data.rng.iloc[bar])>0):
                self.increase+=1
            elif (self.data.rng.iloc[bar-1]>self.data.rng.iloc[bar]) and (np.sign(self.data.rng.iloc[bar-1])<0) and (np.sign(self.data.rng.iloc[bar])<0):
                self.increase -=1
            else:
                self.increase = 0 
        ...
        #OPEN POSITIONS
            ...
            if self.increase==(-1)*bar_seq and (self.data.rng.iloc[bar] > (self.data.ATR.iloc[bar] * atr)):
                if self.position == 0:
                    self.go_long(bar,amount = 'all')
                    self.position = 1
                    self.trailing_stop = self.data.low.iloc[bar] - 20
            ...
            elif self.increase==bar_seq and (self.data.rng.iloc[bar] > (self.data.ATR.iloc[bar] * atr)):
                if self.position == 0:
                    self.go_short(bar,amount = 'all')
                    self.position = -1
                    self.trailing_stop = self.data.high.iloc[bar] + 20
                    self.start_price = self.data.close.iloc[bar]  
                    self.high_start_price = self.data.high.iloc[bar]     
                    self.low_start_price = self.data.low.iloc[bar]   
        ...
        #CLOSE POSITIONS
            if self.position == 1:
                ...
                if ((self.start_price-self.low_start_price+20)<(self.data.close.iloc[bar]-self.start_price)) and self.half_closed==0:
                ...
                elif np.sign(self.data.returns.iloc[bar]) == 1 and ((self.start_price-self.low_start_price+20)<(self.data.close.iloc[bar]-self.start_price)):
                    self.trailing_stop = self.data.low.iloc[bar-1]- 20
                    
            elif self.position == -1:
                #close half, if got profit equaled to stoploss
                if ((self.high_start_price-self.start_price+20)<(self.start_price-self.data.close.iloc[bar])) and self.half_closed==0:
                    ...
                    self.half_closed = 0               
                    #if no actions with position, then pull up stoploss after price climbed over pinbar high + ATR
                elif np.sign(self.data.returns.iloc[bar]) == -1 and ((self.high_start_price-self.start_price+20)<(self.start_price-self.data.close.iloc[bar])):
                    self.trailing_stop = self.data.high.iloc[bar-1] + 20
            ...
            self.data['increase'].iloc[bar] = self.increase
            
        self.close_all(bar+1) # close pos at the last bar       
        

bc = IterativeBacktest("BTCUSDT","2022-08-28","2022-09-28",tf='5m',amount = 1000)

bc.calculate_pinbar_strategy(bar_seq=3, atr = 2)
bc.plot_data()

print('Accuracy is : {}'.format(round(len(bc.data.loc[bc.data['pos_result']==1,['pos_result']])/len(bc.data.loc[bc.data['pos_result']!=0,['pos_result']]),2)))

sf.excel_export(bc.data['2022-09'])
sf.excel_export(bc.data['2022-09-28'], size=100000)
#protestitj dannije parametry na boljshej istorii
#kak ideja, zapustitj perebor parametrov i fititj luchsij rezultat za poslednuju nedelju, i kazhduju nedelju menjatj parametry
#Manual testing
# bc.data.iloc[57]
