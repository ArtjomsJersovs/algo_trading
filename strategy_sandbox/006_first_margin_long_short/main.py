...
class IterativeBacktest(IterativeBase):
    ...
    def calculate_pinbar_strategy(self, lookback_bars=100, ma_interval=20, body_size=0.30, extremum_ratio_size=0.35):
        #print header
        print(75 * "-")
        ...
        over_vol_flag = self.data.volume > self.data.volume_ma
        #around_prev_max_flag = np.where((self.data[f'max_price_{lookback_bars}']>self.data.close) & (self.data[f'max_price_{lookback_bars}']<self.data.high),True,False)
        #around_prev_min_flag = np.where((self.data[f'min_price_{lookback_bars}']<self.data.close) & (self.data[f'min_price_{lookback_bars}']>self.data.low),True,False)
...
bc = IterativeBacktest("BTCUSDT","2022-08-28","2022-09-28",tf='1m',amount = 1000)

bc.calculate_pinbar_strategy()
bc.plot_data()
