...
class Backtester():
    ...
    def plot_results(self):
        if self.results is None:
            ...
            bench_monthly_return = self.results.returns.mean() * 8640 #returns in % monthly
            bench_monthly_var = self.results.returns.std() * np.sqrt(8640) #risk in % monthly #make sense only in normally distributed returns as std and mean is used
            bench_max_drawdown = self.results.bench_drawdown.max()

            strategy_monthly_return = self.results.strategy.mean() * 8640
            strategy_monthly_var = self.results.strategy.std() * np.sqrt(8640) 
            ...
            print('{} trades initiated, in total: {}% on commissions'.format(self.data['trades'].iloc[-1]*2,(self.data['trades'].iloc[-1]*2)*0.075))
            print(75 * "-")
...
tfs = ['5m', '15m','1h','2h','3h','4h','5h','6h','12h']
pairs_tfs_combinations = list(itertools.product(pairs,tfs))
best_combinations = pd.DataFrame()
for i in pairs_tfs_combinations:
    backtest = Backtester(i[0],12,26,9,'2021-01-01','2022-02-01', i[1])
    backtest.optimize_params((5,50,5),(5,50,5),(1,15,2))
