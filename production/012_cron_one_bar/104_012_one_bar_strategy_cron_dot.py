...
class Trader():
    ...
    def __init__(self, ticker, size, interval, hist_period_hours, leverage, ma_interval=15, bb_interval=30, body_size =0.8, sl_coef=1.25, vol_coef = 1):
        ...
        self.size = round_step_size(max(float(size)/self.last_price,0.1), 0.1)
...
if __name__ == "__main__":
    ...
    bot = Trader(ticker='DOTBUSD', size= 40, interval='1h', hist_period_hours=40, leverage=5, ma_interval=20, bb_interval=40, body_size =0.7, sl_coef=2, vol_coef = 1.5)
