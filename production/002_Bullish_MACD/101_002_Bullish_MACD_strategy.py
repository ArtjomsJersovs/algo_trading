...
class Trader():
    ...
    def __init__(self, ticker, interval, hist_period_minutes, fast_ma, slow_ma, smooth=5, pos_size_usd=0):
        ...
        self.file_name = 'C:/Users/Administrator/Documents/GitHub/AJbots/production/002_Bullish_MACD/101_002_Bullish_MACD_tradelog_{}.csv'.format(self.bot_start_date)
    ...
    def callback(self, df):
        ...
        if recent_tick - self.last_print > pd.to_timedelta('1h'):
            printed_price = "101_002 price: {} at: {}".format(round(float(df['k']['c']),0),recent_tick.strftime("%d %b %Y %H:%M"))
    ...
    def define_strategy(self): # "strategy-specific"
        ...
        df['position'] = np.where((df.MACD>0) & (np.sign(df.MACD_hist)>np.sign(df.MACD_hist.shift(1))) & (df.skip_data!=1), 1, np.nan)
        df['position'] = np.where((np.sign(df.MACD_hist)<np.sign(df.MACD_hist.shift(1))) & (df.skip_data!=1), 0, df.position)
    ...
    def execute_trades(self):
        ...
        if self.data["position"].iloc[-1] == 1:
            ...
            #Main order
                self.buy_order = client.order_market_buy(symbol=self.ticker, quantity=round_step_size(float(self.pos_size_usd)/float(self.tick_data['close'].iloc[-1]), 0.001))
            ...
            #Technical fields
                ...
                self.report_trade(self.buy_order, "___________101_002 GOING LONG___________")
            self.position = 1
            ...
            #Main order  
                self.sell_order = client.order_market_sell(symbol=self.ticker, quantity = round_step_size(pd.json_normalize(self.buy_order['fills'])['qty'].apply(pd.to_numeric).sum(), 0.001))
            ...
            #Technical fields
                ...
                self.balance.to_csv(self.bot_balance_filename, index = False)
                self.trades_data.to_csv(self.file_name)
                self.report_trade(self.sell_order, "__________101_002 GOING NEUTRAL_________")
                self.topup_bnb()
            self.position = 0
...
if __name__ == "__main__":
    ...
    bot = Trader(ticker = 'LTCUSDT',interval='5m', hist_period_minutes=100,  fast_ma = 15, slow_ma=10, smooth=3, pos_size_usd=20)
...
# sell_order = client.order_market_sell(symbol='LTCUSDT', quantity = 0.178)
#bot.trades_data.to_csv('C:/Users/Administrator/Documents/GitHub/AJbots/strategy_sandbox/test.csv')

#py Documents\GitHub\AJbots\production\002_Bullish_MACD\101_002_Bullish_MACD_strategy.py
#bot.tick_data


# bot.execute_trades()
# bot.position = 1
# bot.data["position"].iloc[-1] = 0
